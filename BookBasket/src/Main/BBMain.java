...
//				loginInfoP.setBorder(new EtchedBorder(BevelBorder.LOWERED));
		...
		// 새로운 메세지가 있으면 그리고 빌린책이 있으면 출력되게 하자.
				...
				nameL = new JLabel( name + " 님", SwingConstants.CENTER);
...
//				loginStatusP.add(logoutBP, "South"); 	// 버튼을 넣어준다.
		...
		try{
		socket = new Socket("192.168.35.72",8889);
		
		InputStream 	in  = socket.getInputStream();
		OutputStream 	out = socket.getOutputStream();
		BBReceiveThread thread = new BBReceiveThread(this);
		thread.start();
...
//		oin = new ObjectInputStream(socket.getInputStream());
		...
		loginDlg.setVisible(true);
		bookSearchMain.searchAllProc();  // 프로그램 시작 시 전체검색 설정창.
		bookRentalMain.RentalFInitProc(); // 프로그램 시작 시 예약테이블 초기 데이터 설정창
		bookRentalMain.RentalTInitProc(); 
